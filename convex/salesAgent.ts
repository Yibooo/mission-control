...
【旅館業界の代表的な"痛み"（本文で3つ選んで箇条書きにすること）】
- 問い合わせ対応の重複：「同じ質問に何度も返信し、対応が遅れてしまう」
- 口コミ返信の負担：「一軒ずつ丁寧に返したいが、時間が足りない」
- 外国人対応：「英語・中国語の問い合わせに対応できるスタッフがいない」
- 繁忙期の情報発信：「季節プランのSNS投稿・LP文章を書く余裕がない」
- 引き継ぎ・マニュアル：「口頭伝承が多く、スタッフ交代のたびに業務が止まる」
- データ転記：「予約情報をExcelや台帳に手入力する作業が毎日ある」

【作成ルール】
1. subject（件名）:
   - 形式: 【${ryokan.companyName}様】〇〇の負担を、AIで半分にしませんか？
   - 〇〇には旅館固有の最も大きな課題を入れる（例: 口コミ返信、予約対応、多言語対応）
   - 全角30字以内
...
2. body（本文）: 以下の構造を必ず守ること
   ...
   突然のご連絡失礼いたします。
   中小企業向けAI導入支援「AI駆け込み寺」の〇〇と申します。

   ${ryokan.location}でお客様をお迎えされている${ryokan.companyName}様において、このような課題にお悩みではないでしょうか。

   ・[痛み1（旅館に合わせた具体的な課題を「」でくくって記述）]
   ・[痛み2]
   ・[痛み3]

   弊社の「AIスターターパック（¥5,000〜）」では、これらを以下のように解決できます。

   1. [解決策1（痛み1に対応）]
   2. [解決策2（痛み2に対応）]
   3. 自走サポート：「設定が難しくて挫折した」とならないよう、スタッフ様が使いこなせるまで伴走いたします。
...
{"subject":"件名","body":"本文（改行は\\nで表現）"}`;
  ...
  try {
    ...
    // subject・body ともに存在するか確認
    if (!parsed.subject || !parsed.body) throw new Error("フィールド不足");
    return parsed;
  } catch {
    // Gemini 失敗時の高品質フォールバック（同じフォーマットで固定文）
    return {
      subject: `【${ryokan.companyName}様】口コミ返信・予約対応の負担を、AIで半分にしませんか？`,
      body: [
        `${ryokan.companyName} ご担当者様`,
        ``,
        `突然のご連絡失礼いたします。`,
        ...
        `弊社の「AIスターターパック（¥5,000〜）」では、これらを以下のように解決できます。`,
        ``,
        `1. AIによる返信下書き自動生成：過去の回答を学習し、貴館らしい返信案を即座に作成します。`,
        `2. 口コミ対応の効率化：OTAの口コミに対する返信文をAIが自動で下書きします。`,
        `3. 自走サポート：スタッフ様が使いこなせるまで伴走いたします。`,
        ``,
        `「現場で本当に使えるAI」を一緒に構築させてください。`,
        `まずは30分の無料相談で、${ryokan.companyName}様の業務の中から「どこをAIに任せられるか」を診断させていただきます。`,
        ``,
        `━━━━━━━━━━━━━━━━━━━━`,
        `AI駆け込み寺`,
